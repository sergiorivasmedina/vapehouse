...
const _require3 = require('country-language'),
      getLanguage = _require3.getLanguage;

const _require4 = require('lodash'),
      isEmpty = _require4.isEmpty,
      sortBy = _require4.sortBy,
      every = _require4.every,
...
class I18N {
  ...
  middleware(ctx, next) {
    ...
    ctx.pathWithoutLocale = locale ? ctx.path.slice(`/${locale}`.length) : ctx.path;
    ...
    if (locale !== ctx.state.locale) {
      ...
      return ctx.redirect(`/${ctx.state.locale}${ctx.pathWithoutLocale}${isEmpty(ctx.query) ? '' : `?${stringify(ctx.query, this.config.stringify)}`}`);
    ...
    ctx.state.availableLanguages = sortBy(locales.map(locale => {
      return {
        locale,
        url: `/${locale}${ctx.pathWithoutLocale}${isEmpty(ctx.query) ? '' : `?${stringify(ctx.query, this.config.stringify)}`}`,
  ...
  redirect(ctx, next) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!hasLang) {
        ...
        if (!isEmpty(ctx.query)) redirect += `${stringify(ctx.query, _this.config.stringify)}`;
